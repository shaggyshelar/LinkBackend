using System;
using Microsoft.VisualStudio.TestTools.UnitTesting;

using System.Web.Mvc;
using System.Collections.Generic;
using Espl.Linkup.Web.Controllers.Leaves;
using System.Web.Http.Results;
using Espl.Linkup.Web.Controllers.Corporate;
using Espl.Linkup.Domain.Features;
using Espl.Linkup.Domain.Profile.Users;
using Espl.Linkup.Domain.Departments;

namespace Espl.Linkup.Web.Tests.Controllers
{
    [TestClass]
    public class FeatureControllerTest
    {
        [TestMethod]
        public void getAllFeature()
        {
            // Arrange
            FeatureController controller = new FeatureController();

            var actResult = controller.Get();
            // Act
            var result = actResult as OkNegotiatedContentResult<List<Feature>>;

            // Assert
            Assert.IsNotNull(result);

            Assert.IsTrue(result.Content.Count >= 1);
        }


        [TestMethod]
        public void getFeature()
        {
            // Arrange
            FeatureController controller = new FeatureController();

            var actResult = controller.Get(1);
            // Act
            var result = actResult as OkNegotiatedContentResult<Feature>;

            // Assert
            Assert.IsNotNull(result);
            Assert.IsTrue(result.Content.ID == 1);
        }


        [TestMethod]
        public void PostFeature()
        {
            // Arrange
            FeatureController controller = new FeatureController();

            Feature FeatureObj = new Feature
            {
                User = new User { ID = 1 },
                Name = "New Feature",
            };
            var actResult = controller.Post(FeatureObj);
            // Act
            var result = actResult as OkNegotiatedContentResult<Feature>;

            // Assert
            Assert.IsNotNull(result);
            Assert.IsTrue(result.Content.ID > 0);
        }



        [TestMethod]
        public void PutFeature()
        {
            // Arrange
            FeatureController controller = new FeatureController();

            Feature FeatureObj = new Feature
            {
                User = new User { ID = 1 },
                Name = "Put request sucessfull",
                ID = 1
            };
            var actResult = controller.Put(1, FeatureObj);
            // Act
            var result = actResult as OkNegotiatedContentResult<Feature>;

            // Assert
            Assert.IsNotNull(result);
            Assert.IsTrue(result.Content.Name.Equals("Put request sucessfull"));
        }



        [TestMethod]
        public void DeleteFeature()
        {
            // Arrange
            FeatureController controller = new FeatureController();

            var actResult = controller.Delete(1);
            // Act
            var result = actResult as OkNegotiatedContentResult<bool>;

            // Assert
            Assert.IsNotNull(result);
            Assert.IsTrue(result.Content == true);
        }

    }
}
